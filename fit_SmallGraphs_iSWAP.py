...
from model.MLP_NEW_F import GBFCN2_iSWAP_simple_AiHao_v2_global
...
def fit_smallgraphs_iSWAP(print_or_plot_or_savemodel=True, arg=None, saving_dir=None):
    ...
    model = GBFCN2_iSWAP_simple_AiHao_v2_global(arg.hidden_dim, arg.num_layers, arg.dropout,
    ...
    loaded_datas = [(torch.tensor(load_pkl(f'Datasets_FIN/{graph_names[graph_adj_mat_id]}/state_FIN.pkl')),
                     torch.tensor(load_pkl(f'Datasets_FIN/{graph_names[graph_adj_mat_id]}/score_node_FIN.pkl')),
    ...
    loaded_datas = [(
        ...
        torch.tensor(data[1]),
        torch.tensor(data[2])[:, :, :adj_mat.shape[0]].to(dtype).to(dev))
...
def transfer_smallgraphs(gate_type='iSWAP', pretrained_path=None, arg=None,
    ...
    model = GBFCN2_iSWAP_simple_AiHao_v2_global(arg.hidden_dim, arg.num_layers, arg.dropout,
                                                arg.xw_in_dim, arg.adj_in_dim, arg.global_in_dim)
    # model = GBFCN2_iSWAP_simple_AiHao(dropout=arg.dropout, hidden_layers=arg.hidden_dim)
    ...
    loaded_datas = [(torch.tensor(normalize_ops(data[0], [-8, -8], [8, 8])).to(dtype).to(dev),
                     torch.tensor(data[1]),
                     torch.tensor(data[2])[:, :, :qubit_num].to(dtype).to(dev)) for data in loaded_datas]
    ...
    data_in_1, data_in_2, data_out = loaded_datas[0]
    data_in_2 = data_in_2[:data_in_1.shape[0]]
    embedder = graph_data_embedders[0]

    embedded_in = embedder.bw_datain_to_modelin_iSWAP(data_in_1, data_in_2)
    embedded_out = embedder.bw_dataout_to_modelout_iSWAP(data_out)
    ...
    Sp, p_val = safe_spearman(embedded_out.detach().cpu().numpy().reshape(-1), pred.detach().cpu().numpy().reshape(-1))
    ...
    iSWAP_ct = embedder.bw_modelout_to_dataout_iSWAP(embedded_out)
    all_Sp, all_p = safe_spearman(iSWAP_ct.sum(-1).mean(-1).detach().cpu().numpy(),
                                  iSWAP_ct_pre.sum(-1).mean(-1).detach().cpu().numpy())
    all_corr, all_p_c = pearsonr(iSWAP_ct.sum(-1).mean(-1).detach().cpu().numpy(),
                                 iSWAP_ct_pre.sum(-1).mean(-1).detach().cpu().numpy())
    all_MAE = torch.abs(iSWAP_ct.sum(-1).mean(-1) - iSWAP_ct_pre.sum(-1).mean(-1)).mean().item()
    all_MAE_r = (torch.abs(iSWAP_ct.sum(-1).mean(-1) - iSWAP_ct_pre.sum(-1).mean(-1))/iSWAP_ct.sum(-1).mean(-1)).mean().item()
...
def main(arg):
    ...
    fit_smallgraphs_iSWAP(arg=arg, saving_dir=saving_dir)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gate_type', type=str, default='iSWAP', help='')
    ...
    parser.add_argument('--xw_in_dim', type=int, default=35, help='')
    parser.add_argument('--adj_in_dim', type=int, default=12, help='')
